 ...
 */
const deserializeAppStateSyncKey = (value) => {
    const appStateSyncKeyData = proto.Message.AppStateSyncKeyData;
    if (typeof appStateSyncKeyData.fromObject === 'function') {
        return appStateSyncKeyData.fromObject(value);
    }
    if (typeof appStateSyncKeyData.create === 'function') {
        return appStateSyncKeyData.create(value);
    }
    return value;
};
/**
 ...
 */
const deleteKeysByPattern = async (redis, pattern, logger) => {
    let cursor = 0;
    do {
            ...
            keys: {
                get: async (type, ids) => {
                    ...
                    await Promise.all(ids.map(async (id) => {
                        ...
                        if (value) {
                            data[id] = (type === 'app-state-sync-key' ? deserializeAppStateSyncKey(value) : value);
                        }
                    }));
                    return data;
                },

...
exports.listSessions = exports.listHSetSessions = exports.deleteKeysWithPattern = exports.useRedisAuthState = exports.deleteHSetKeys = exports.useRedisAuthStateWithHSet = void 0;
 ...
 */
const useRedisAuthStateWithHSet = async (redisOptions, sessionId = 'session', logger) => {
    ...
    const writeData = async (key, data) => {
        await redis.hset(createKey('auth', sessionId), key, JSON.stringify(data, baileys_1.BufferJSON.replacer));
    };
 ...
 */
const deleteHSetKeys = async ({ redis, sessionId, logger }) => {
    try {
        logger === null || logger === void 0 ? void 0 : logger('Removing auth state for session:', sessionId);
        await redis.del(createKey('auth', sessionId));
    }
    catch (err) {
        const error = err;
        logger === null || logger === void 0 ? void 0 : logger('Error deleting session:', error.message);
        throw error;
    }
};
 ...
 */
const useRedisAuthState = async (redisOptions, sessionId = 'session', logger) => {
    var _a;
    const redis = new ioredis_1.default(redisOptions);
    redis.on('connect', async () => {
        const redisClientName = `baileys-auth-${sessionId}`;
    ...
    const writeData = async (key, data) => {
        await redis.set(createKey(key, sessionId), JSON.stringify(data, baileys_1.BufferJSON.replacer));
    };
    const readData = async (key) => {
 ...
 */
const deleteKeysWithPattern = async ({ redis, sessionId, logger }) => {
    try {
        ...
        do {
            ...
            if (keys.length > 0) {
                ...
                logger === null || logger === void 0 ? void 0 : logger(`Deleted ${keys.length} keys for session: ${sessionId}`);
            }
        } while (cursor !== 0);
    }
    catch (err) {
 ...
 */
const listHSetSessions = async ({ redis, logger }) => {
    try {
        const sessions = [];
        let cursor = 0;
        logger === null || logger === void 0 ? void 0 : logger('Scanning for Hash-based sessions...');
        do {
        ...
        logger === null || logger === void 0 ? void 0 : logger(`Found ${sessions.length} Hash-based sessions`);
        return sessions;
    }
    catch (err) {
        const error = err;
        logger === null || logger === void 0 ? void 0 : logger('Error listing sessions:', error.message);
        throw error;
    }
};
exports.listHSetSessions = listHSetSessions;
/**
 ...
 */
const listSessions = async ({ redis, logger }) => {
    try {
        const sessionsSet = new Set();
        let cursor = 0;
        logger === null || logger === void 0 ? void 0 : logger('Scanning for key-value based sessions...');
        do {
        ...
        logger === null || logger === void 0 ? void 0 : logger(`Found ${sessions.length} key-value based sessions`);
        return sessions;
    }
    catch (err) {
        const error = err;
